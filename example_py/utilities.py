...
class DeviceConnection:
    ...
    UDP_PORT = 10001  # for low level API
    ...
    def __init__(self, ip_address, port=TCP_PORT, credentials=("", "")):
        ...
        self.transport = TCPTransport() \
            if port == DeviceConnection.TCP_PORT \
            else UDPTransport()  # to assembly Send High-Level(HL) commands
        self.router = RouterClient(self.transport, RouterClient.basicErrorCallback)  # to send HL commands
    ...
    def __enter__(self):
        ...
        if self.credentials[0] != "":  # create session
            ...
            self.sessionManager = SessionManager(self.router) # to send the messages
